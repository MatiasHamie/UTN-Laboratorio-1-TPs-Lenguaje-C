...
void getDate(int* dia, int* mes, int* anio){
    ...
    validarMes=getInt(mes,"Ingrese mes","Mes invalido [RANGO 1-12]",1,12);
    while(validarMes==-1){
       validarMes=getInt(mes,"Ingrese mes","Mes invalido [RANGO 1-12]",1,12);
    }
    switch (*mes){
        ...
        case 10 :
        case 12 :
            validarDia=getInt(dia,"Ingrese dia","Dia invalido [Rango 1-31]",1,31);
            while(validarDia==-1){
                validarDia=getInt(dia,"Ingrese dia","Dia invalido [Rango 1-31]",1,31);
            }
            break;
        ...
        case  9 :
        case 11 :
            validarDia=getInt(dia,"Ingrese dia","Dia invalido [Rango 1-30]",1,30);
            while(validarDia==-1){
                validarDia=getInt(dia,"Ingrese dia","Dia invalido [Rango 1-30]",1,30);
            }
           break;
        case  2 :
            validarDia=getInt(dia,"Ingrese dia","Dia invalido [Rango 1-30]",1,28);
            while(validarDia==-1){
                validarDia=getInt(dia,"Ingrese dia","Dia invalido [Rango 1-30]",1,28);
            }
            break;
    }
    validarAnio=getInt(anio,"Ingrese anio","Anio invalido [Rango 1900-2019]",1900,2019);
    while(validarAnio==-1){
        validarAnio=getInt(anio,"Ingrese anio","Anio invalido [Rango 1900-2019]",1900,2019);
    }
}
