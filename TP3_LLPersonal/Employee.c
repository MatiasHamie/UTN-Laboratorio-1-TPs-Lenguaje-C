...
int menu(){
    ...
    printf("10. Salir\n");
    printf("11. Duplicar sueldo empleados\n");
    printf("12. Mostrar empleados con inicial M\n");

    validacion=getInt(&opcion,"Ingrese opcion","ERROR, Re",1,12);
    while(validacion==-1){
        validacion=getInt(&opcion,"Ingrese opcion","ERROR, Re",1,12);
    }
...
void* map_duplicarSueldo(void* pEmp){
    Employee* pAuxElement=NULL;

    if(pEmp!=NULL){
        //Creo un empleado y lo inicializo
        pAuxElement=employee_new();

        //El emp recibido lo casteo a Employee
        Employee* pCastEmp=(Employee*)pEmp;

        //Creo variables auxiliares para potencial empleado
        int auxId=0;
        int auxHoras=0;
        char auxNombre[21]={"a"};
        float auxSueldo=0;

        //Obtengo los campos del empleado recibido y las guardo
        employee_getId(pCastEmp,&auxId);
        employee_getNombre(pCastEmp,auxNombre);
        employee_getHorasTrabajadas(pCastEmp,&auxHoras);
        employee_getSueldo(pCastEmp,&auxSueldo);

        //Guardo las variables del empleado recibido y lo pego en uno nuevo
        employee_setId(pAuxElement,auxId);
        employee_setNombre(pAuxElement,auxNombre);
        employee_setHorasTrabajadas(pAuxElement,auxHoras);
        employee_setSueldo(pAuxElement,auxSueldo*2);
    }
    return (void*)pAuxElement;
}

int filter_Iniciales(void* pProd){
    int returnAux= 0;
    Employee* pEmp=(Employee*)pProd;
    if(pEmp!= NULL){
        if(pEmp->nombre[0]=='M'){
            returnAux=1;
        }
